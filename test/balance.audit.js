...
if(require.main===module){
 ...
 fs.mkdirSync(path.join(root,'test','reports'),{recursive:true});fs.writeFileSync(path.join(root,'test','reports','balance-results.json'),JSON.stringify({method:{seeds,duration,dt,description:'Actual game loop, stub DOM/audio/render; stationary and sinusoidal bots, automatic EMP and ranked offered upgrades; late stage starts with weapon 3 / damage 2, not a natural playthrough',lateStart:'wave 20 for endless/roguelike; level 8 wave 1 for campaign; no accumulated rogue upgrades',layout:'Mock HUD bottom 100px without Boss / 155px with Boss; 390x844 and 900x760; no human reaction model'},results},null,2));
 const probes=[];for(const mode of ['endless','campaign','roguelike'])for(const d of ['easy','normal','hard']){const r=make(1);r.api.start(mode,d,true);probes.push({mode,difficulty:d,...r.api.probe()});}console.log('PROBES',JSON.stringify(probes));fs.writeFileSync(path.join(root,'test','reports','balance-probes.json'),JSON.stringify(probes,null,2));
 ...
 fs.writeFileSync(path.join(root,'test','reports','balance-perks.json'),JSON.stringify(perks,null,2));
 const hashes={};for(const f of ['core','logic','bosses','entities','game','input'])hashes['js/'+f+'.js']=require('crypto').createHash('sha256').update(fs.readFileSync(path.join(root,'js',f+'.js'))).digest('hex');fs.writeFileSync(path.join(root,'test','reports','balance-source-hashes.json'),JSON.stringify(hashes,null,2));
