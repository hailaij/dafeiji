...
for(const ship of ['falcon','bulwark','wisp'])for(const weaponId of ['pulse','heavy','fan'])for(const mode of ['endless','campaign','roguelike'])for(const difficulty of ['easy','normal','hard']){
 for(const width of [390,900])for(let seed=1;seed<=2;seed++){
  const r=make(seed,width,width===390?844:760);r.api.start(mode,difficulty,true,ship,weaponId);let target=width/2,seconds=0;
  ...
  const s=r.api.peek();results.push({ship,weaponId,mode,difficulty,width,seed,alive:s.STATE!=='gameover',seconds,wave:s.wave,level:s.level,...r.stats()});
 }
 const a=results.filter(r=>r.ship===ship&&r.weaponId===weaponId&&r.mode===mode&&r.difficulty===difficulty);console.log(ship,weaponId,mode,difficulty,a.filter(r=>r.alive).length+'/'+a.length);
}
fs.writeFileSync('test/reports/loadouts/late-reactive.json',JSON.stringify({method:'Late-stage fixed equipment; 324 runs, two seeds, two viewports; heuristic visible-bullet/telegraph avoidance, enemy alignment and nearby loot; automatic EMP; not human win rates',results},null,2));
